...
async function deactivateEmployeeOnApi(employeeId: string, updatedByUserId: string): Promise<void> {
...
export default function EmployeesPage() {
  ...
  const handleDeactivateEmployee = async (employee: EmployeeViewModel) => {
    const employeeName = employee.fullName || `${employee.lastName} ${employee.firstName}`.trim() || "Сотрудник";
    const confirmed = window.confirm(`Деактивировать сотрудника "${employeeName}"?`);
    if (!confirmed) return;
    ...
    try {
      ...
      const authUser = getStoredAuthUser();
      await deactivateEmployeeOnApi(employeeId, authUser?.userId || "");

      const nextEmployees = employees.filter((item) => item.userId !== employeeId);
      setEmployees(nextEmployees);
      showToast("Сотрудник деактивирован.");
    } catch (error) {
      console.error("Ошибка деактивации сотрудника:", error);
      showToast(error instanceof Error ? error.message : "Не удалось деактивировать сотрудника.", "info");
    } finally {
                    ...
                    <td>
                      <div className="action-buttons">
                        ...
                        <button
                          ...
                          disabled={deactivatingEmployeeId === employee.userId}
                        >
                          <SvgIcon name="delete" />
                        </button>
                      </div>
